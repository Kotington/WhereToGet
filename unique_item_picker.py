...
def parse_items(language="en"):
    if language == "ru":
        file_path = resource_path("Last Epoch uniques (ru).txt")
        obtained_header = "На кого охотиться:"
        not_found = "Не найдено"
    else:
        file_path = resource_path("Last Epoch uniques.txt")
        obtained_header = "Obtained from:"
        not_found = "Not found"
    try:
    ...
    items = []
    for idx, block in enumerate(blocks):  # используем индекс как уникальный ID
        lines = [line.strip() for line in block.splitlines()]
        if len(lines) < 3:
            continue
        name = lines[0]
        item_type = lines[1] if len(lines) > 1 else "Unknown"
        unique_name = lines[2] if len(lines) > 2 else "No Unique Name"
        try:
            obtained_start = lines.index(obtained_header) + 1
            obtained = lines[obtained_start:] or [not_found]
        except ValueError:
            obtained = [not_found]
        items.append({
            "id": idx,
...
class UniquePickerApp:
    ...
    def update_left_list(self, items):
        ...
        for item in items:
            ...
            btn.bind("<Button-1>", lambda e, item=item: self.add_to_selected(item))
    ...
    def update_right_panel(self):
        ...
        for item in self.selected_items:
            ...
            if item["unique_name"]:
                unique_name = tk.Label(
                    content_frame,
                    text=item["unique_name"],
                    bg="#2c2c2c",
                    fg="lightgray",
                    anchor="w",
                    wraplength=wraplength_px,
                ...
                unique_name.pack(anchor="w", pady=(1, 0))
                label_list.append(unique_name)
            ...
            # Все виджеты, по которым можно кликнуть
            clickables = [card_frame, content_frame, title, type_label]
            if item["unique_name"]:
                clickables.append(unique_name)
